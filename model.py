...
def get_model(node_size,rel_size,node_hidden,rel_hidden,triple_size,batch_size,n_attn_heads = 2,dropout_rate = 0,gamma = 3,lr = 0.005,depth = 1):
    adj_input = Input(shape=(None,2))
    index_input = Input(shape=(None,2),dtype='int64')
    val_input = Input(shape=(None,))
    rel_adj = Input(shape=(None,2))
    ent_adj = Input(shape=(None,2))
    rel_mark = Input(shape=(None,2))
